...
> **Projet** : Iris ML Demo — Full Stack App (Flutter + FastAPI + Jupyter)
>
...
Ce dataset est maintenant prêt pour l'entraînement d'un modèle de classification. Dans notre projet, le notebook `notebook/train_model.ipynb` utilise un **Random Forest Classifier** pour entraîner le modèle qui est ensuite exposé via l'API FastAPI.
